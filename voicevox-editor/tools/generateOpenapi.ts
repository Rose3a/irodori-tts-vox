...
async function updateOpenapiJson(engineBase: string) {
  ...
  const response = await fetchJson(openapiUrl) as any;
  // Irodori extensions are part of the checked-in source contract. Keep this
  ...
  audioQuery.properties.irodori_caption = { type: "string" };
  audioQuery.properties.irodori_reference_audio = {
    type: "object",
    properties: {
      dataUrl: { type: "string" },
      mime: { type: "string" },
