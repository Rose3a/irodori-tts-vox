<template>
  <div v-if="query" class="root full-height q-py-md" data-testid="AudioInfo">
    <IrodoriSettings :engineId="audioItem.voice.engineId" :activeAudioKey="activeAudioKey" />
...
import IrodoriSettings from "@/components/Talk/IrodoriSettings.vue";
...
const parameterConfigs = computed<ParameterConfig[]>(
  () =>
    ...
    ].filter(({ key }) => key === "speedScale") as ParameterConfig[],
);
