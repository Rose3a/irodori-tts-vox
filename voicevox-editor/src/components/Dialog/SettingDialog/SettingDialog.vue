          ...
          <div class="container">
            <BaseScrollArea>
              ...
              <div class="setting-card">
                <h5 class="headline">立ち絵と口パク</h5>
                <SelectCell
                  v-model="lipSyncModeComputed"
                  title="口パクの精度"
                  description="簡易は通常/口パクの2枚を開閉で切り替えます。母音は『あいうえお』の口パーツがある立ち絵向けで、セリフのモーラに合わせて口の形を切り替えます（パーツが無い場合は簡易と同じ表示に戻ります）。"
                  :options="lipSyncModeOptions"
                />
                <SelectCell
                  v-model="lipSyncSpeedComputed"
                  title="口の切り替え速度"
                  description="速いほど細かく反応します。遅いと口の動きが滑らかになり、速いと子音の切り替わりが見えます。"
                  :disable="lipSyncModeComputed === 'off'"
                  :options="lipSyncSpeedOptions"
                />
                <SelectCell
                  v-model="lipSyncBlinkComputed"
                  title="瞬き"
                  description="立ち絵の瞬き（まばたき）の頻度です。"
                  :options="lipSyncBlinkOptions"
                />
                <SelectCell
                  v-model="lipSyncTimelineComputed"
                  title="口パクのタイムライン"
                  description="推定はセリフの文字数から発話時刻を割り振ります（ASR不要・即時）。ASRは生成した音声から実際の発話時刻を測るので、間の取り方や伸ばしまで口が追従します（エンジン側にASRモデルが必要・再生する音声を使います）。"
                  :disable="lipSyncModeComputed === 'off'"
                  :options="lipSyncTimelineOptions"
                />
                <BaseRowCard
                  title="母音パーツの置き方"
                  description="話者フォルダに『&lt;名前&gt;_mouth_a.png』〜『_mouth_o.png』と『&lt;名前&gt;_mouth_n.png』を置くと、母音モードで個別の口に切り替わります。無い場合は『&lt;名前&gt;_open.png』を使います。"
                />
              </div>
              <!-- Irodori runtime card -->
              <div class="setting-card">
                <h5 class="headline">kataribe の動作設定</h5>
              ...
              <!-- Irodori compatibility notice -->
              <div class="setting-card">
                <h5 class="headline">互換機能</h5>
                <BaseRowCard
                  title="本家エディタ向け機能"
                  description="アクセント辞書、モーフィング、歌唱、マルチエンジン、利用状況データ送信は、この kataribe では利用しません。"
                />
              </div>
              <!-- Original upstream settings are intentionally hidden in kataribe. -->
...
import IrodoriGlobalSettings from "@/components/Talk/IrodoriGlobalSettings.vue";
...
// 立ち絵の口パク設定
const [lipSyncMode, setLipSyncMode] = useRootMiscSetting(store, "lipSyncMode");
const [lipSyncSpeed, setLipSyncSpeed] = useRootMiscSetting(
  store,
  "lipSyncSpeed",
);
const [lipSyncBlink, setLipSyncBlink] = useRootMiscSetting(
  store,
  "lipSyncBlink",
);
const lipSyncModeOptions = [
  { label: "なし", value: "off" },
  { label: "簡易（開閉のみ）", value: "simple" },
  { label: "母音（あいうえお）", value: "vowel" },
];
const lipSyncSpeedOptions = [
  { label: "ゆっくり", value: "slow" },
  { label: "標準", value: "normal" },
  { label: "速い", value: "fast" },
  { label: "最速", value: "max" },
];
const lipSyncBlinkOptions = [
  { label: "なし", value: "off" },
  { label: "標準", value: "normal" },
  { label: "多め", value: "often" },
];
const [lipSyncTimeline, setLipSyncTimeline] = useRootMiscSetting(
  store,
  "lipSyncTimeline",
);
const lipSyncTimelineOptions = [
  { label: "推定（文字数から）", value: "mora" },
  { label: "ASR（発話時刻から）", value: "asr" },
];
const lipSyncTimelineComputed = computed({
  get: () => lipSyncTimeline.value,
  set: (value: string) => setLipSyncTimeline(value as "mora" | "asr"),
});
const lipSyncModeComputed = computed({
  get: () => lipSyncMode.value,
  set: (value: string) => setLipSyncMode(value as "off" | "simple" | "vowel"),
});
const lipSyncSpeedComputed = computed({
  get: () => lipSyncSpeed.value,
  set: (value: string) =>
    setLipSyncSpeed(value as "slow" | "normal" | "fast" | "max"),
});
const lipSyncBlinkComputed = computed({
  get: () => lipSyncBlink.value,
  set: (value: string) => setLipSyncBlink(value as "off" | "normal" | "often"),
});
