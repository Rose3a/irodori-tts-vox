  ...
  <div class="detail">
    <BaseScrollArea>
      <div class="inner">
        ...
        <div class="form-row">
          <h3 class="headline">
            {{ isIrodori ? "読みの確認" : "アクセント調整" }}
          </h3>
          <div v-if="isIrodori">
            登録した読みを優先して変換します。アクセントは自動生成です。
          </div>
          <div v-else>
          ...
          <div>
            <BaseButton
              ...
              :disabled="uiLocked || nowGenerating || !isValid"
              :icon="nowPlaying ? 'stop' : 'play_arrow'"
              @click="nowPlaying ? stop() : play()"
            />
          </div>
          <div
            v-if="accentPhrase && !isIrodori"
...
const isValid = computed(() => {
  return (
    ...
    (isIrodori.value || accentPhrase.value != undefined)
  );
});

const computeRegisteredAccent = () => {
  if (isIrodori.value) return 0;
...
const isIrodori = computed(
  () =>
    store.state.engineManifests[voiceComputed.value.engineId]?.brandName ===
    "Irodori-TTS",
);
...
const setYomi = async (text: string) => {
  ...
  if (isIrodori.value) yomi.value = text;
};
