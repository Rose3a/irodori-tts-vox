...
export function filterNonEmptyAudioKeys<T extends string>(
  audioKeys: readonly T[],
  audioItems: Record<string, { text: string }>,
): T[] {
  return audioKeys.filter((audioKey) => audioItems[audioKey]?.text.trim().length > 0);
}
  ...
  constructor(
    ...
    this.addEventListener("generatestart", async (e) => {
      try {
        ...
        this.dispatchEvent(new GenerateErrorEvent(e.audioKey, error));
      }
    });
    ...
    this.addEventListener("generateerror", (e) => {
      delete this.generating;
      this.dispatchEvent(new WaitEndEvent(e.audioKey));
      if (this.finished) return;
      ...
      if (next) {
        this.dispatchEvent(new GenerateStartEvent(next));
      } else if (!this.playing && this.playQueue.length === 0) {
        this.finish();
      }
    });
...
interface ContinuousPlayerEvents {
  ...
  generateerror: GenerateErrorEvent;
